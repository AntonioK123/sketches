...
function keyPressed() {
  if (key === "a" || key === "A") {
    ...
    sk.save(`Effets de soir (${fxhash}).png`);
  }
  if (key === "s" || key === "S") {
...
const backgroundGradient = (sk) => {
  if (browser === "chrome") {
    let grad = drawingContext.createRadialGradient(0, 0, 100, 100, 100, 800);
    grad.addColorStop(1, hexToRgba(random(palette))); //Donji dio canvasa boja
    grad.addColorStop(0.2, hexToRgba("#000")); //Srednji dio canvasa boja
    ...
    let grad = drawingContext.createRadialGradient(0, 0, 0, 100, 600, 1000);
    grad.addColorStop(1, hexToRgba(random(palette))); //Donji dio canvasa boja
    grad.addColorStop(0.2, hexToRgba("#000")); //Srednji dio canvasa boja
    grad.addColorStop(0, hexToRgba(random(palette))); // Gornji dio canvasa boja
    sk.drawingContext.fillStyle = grad;
...
const texture2 = (sk) => {
  ...
  for (i = 0; i < 1000; i++) {
    ...
    for (m = 0; m < 1000; m += 100) {
      ...
      let r = (2 / sqrt(random())) * 60;
      let th = random(TWO_PI);
      let x = cos(th) * r * random(2);
      let y = sin(th) * r * random(3);
